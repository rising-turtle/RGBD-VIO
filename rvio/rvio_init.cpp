...
bool RVIO::initialStructure()
{
    ...
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)
    {
        ...
        {
            ROS_DEBUG("RVIO_init.cpp: solve pnp fail!");
            return false;
        }
        cv::Rodrigues(rvec, r);
...
bool RVIO::visualInitialAlignWithDepth()
{
    ...
    if(!result)
    {
        ROS_ERROR("RVIO_init.cpp: solve g failed!");
        return false;
    }
...
bool RVIO::visualInitialAlign()
{
    ...
    ROS_DEBUG("RVIO_init.cpp: before set feature depth!"); 
    ...
    ROS_INFO("RVIO_init.cpp: after clear feature depth"); 
    ...
    ROS_DEBUG("RVIO_init.cpp: before triangulate!"); 

    // f_manager.triangulate(Ps, &(TIC_TMP[0]), &(RIC[0]));
    f_manager.triangulate(Ps, Rs, &(TIC_TMP[0]), &(RIC[0]));

    ROS_INFO("RVIO_init.cpp: after triangulate!"); 
